...
if cmode==2,  % w cross b control
  % submit your code here with your name on it certifying that is your own
  % work.
  % The previous section (Bdot bang-bang controller) was modified by Anant
  % Girdhar on October 09, 2024 to implement an omega cross b controller
  tlast=0; Bxlast=0; Bylast=0; Bzlast=0; mxlast=0; mylast=0; mzlast=0;
  %
  for i = 2:N
  %
  ...
  %
    if (ti/sample_rate-floor(ti/sample_rate))<0.001, % test to see if this is a control update (1 msec test threshold)
      if (ti~=tlast),    % execute if this is not the first sample
        ...
        Bdoty=(By_body(1)-Bylast)/(ti-tlast);
        Bdotz=(Bz_body(1)-Bzlast)/(ti-tlast);
        % Implement the wxb controller
        k = 1;
        mvec_body = k * cross(w, Bvec_body) / vecnorm(Bvec_body)^2;
        % Ensure that the dipole doesn't exceed the maximum value
        mvec_magnitude = vecnorm(mvec_body);
        if mvec_magnitude > moment
          mvec_body = mvec_body * moment / mvec_magnitude;
        end
        torque_body=cross(mvec_body,Bvec_body);
        mxlast=mvec_body(1);
        mylast=mvec_body(2);
